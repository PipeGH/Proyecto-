   ...
   public List listar(){
      ...
      List<Invitado>lista= new ArrayList<>(); 
        ...
        public int agregar(Invitado in){
            String sql="insert into invitados(id_invitado,nombre,apellido)values(?,?,?)";
            try{
            con=cn.Conexion();
            ps=con.prepareStatement(sql);
        ...
        String sql="update invitados set nombre=?,apellido=? where id_invitado=?";
            try{
            con=cn.Conexion();
            ps=con.prepareStatement(sql);
            
            ps.setInt(3, in.getId());
